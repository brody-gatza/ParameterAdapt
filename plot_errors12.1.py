...
# Set to None to start from the first available iteration.
# Set to a number to start plotting/computing from that iteration.
#
# Example:
# MIN_ITERATION_TO_INCLUDE = 10000
MIN_ITERATION_TO_INCLUDE = None

# Set to None to continue through the final available iteration.
# Set to a number to stop plotting/computing at that iteration.
#
# Example:
# MAX_ITERATION_TO_INCLUDE = 50000
MAX_ITERATION_TO_INCLUDE = 20000
...
# Set to 1 to plot every point.
# Set to 5, 10, 20, etc. to plot fewer points and reduce PDF file size.
#
# This only down-samples the plotted curves. It does not down-sample the data
# before computing moving averages, slopes, cumulative sums, etc.
...
# Temporarily disable custom y-axis limiting for slope-related plots.
# Main data plots and moving-average comparison plots still use auto limits.
APPLY_AUTO_Y_LIMITS_TO_SLOPE_PLOTS = False

Y_LIMIT_PADDING_FRACTION = 0.08

# Fully automatic robust y-axis limiting.
#
# This only affects display limits. It does not delete, clip, or modify data.
AUTO_Y_LIMITS_IGNORE_EXTREME_OUTLIERS = True

# For log-y plots, non-positive values are ignored automatically.
# Leave as None for automatic behavior.
...
def get_automatic_endpoint_trim_count(num_points):
    """
    Automatically decide how many points to ignore at each end while estimating
    y-axis limits.

    This does not remove those points from the plot.

    The trim is intentionally conservative:
      - 0 points for short signals.
      - Up to 2 percent of data on each end.
      - At most 100 points on each end.
      - Never trims so much that fewer than about 10 points remain.
    """
...
def automatically_trim_line_endpoints(y):
    """
    Remove a small automatically-selected number of endpoint samples for
    y-limit estimation only.

    The plotted data itself is unchanged.
    """
...
def robust_center_and_scale(y):
    ...
    The factor 1.4826 converts MAD to an estimate comparable to standard
...
def get_percentile_limits_automatic(y):
    """
    Fallback percentile limiter.

    The percentile range is selected automatically from the amount of data.
    Larger datasets can safely ignore smaller fractional tails.
    """
...
def get_robust_automatic_y_limits(y):
    """
    Determine automatic robust limits for a 1D array.

    This function assumes that log-transformed data has already been passed
    in for log-y plots.

    Strategy:
      1. Use median/MAD to detect extreme outliers.
      2. If MAD is useful, keep non-extreme values.
      3. If MAD fails, use automatic percentile fallback.
      4. If all else fails, use min/max.
    """
...
def pad_y_limits(
    ...
    For log-y plots, padding is applied in log10-space.
    """
...
def apply_adaptive_y_limits(
    ...
    """
    Apply automatic robust y-axis limits.

    This version handles:
      - noisy data,
      - isolated startup spikes,
      - isolated shutdown spikes,
      - log-y plots,
      - multiple lines on one axis.

    Endpoint trimming is applied per plotted line before combining data.

    The plotted data is unchanged.
    """
