...
# Plot slope of raw data on the slope figure
PLOT_SLOPE_OF_RAW = True

# Plot slope of moving-average data on the slope figure
...
def make_slope_output_file(output_file, output_suffix=""):
    """
    Create a slope output filename.

    Example:
        prim_max_comparison.png, _MA_100
        -> prim_max_comparison_slope_MA_100.png
    """
    output_file = Path(output_file)

    return str(
        output_file.with_name(
            f"{output_file.stem}_slope{output_suffix}{output_file.suffix}"
        )
    )
...
def plot_slope_files(
    ...
    """
    Plot slope figures for one or more files.

    Each variable is plotted in its own subplot.
    Files listed together are overlaid on the same axes.

    This slope figure plots:
        slope of raw data
    ...
    for ax, var_index in zip(axes, variable_indices):
        ...
        for file_idx, (filename, iterations, variables) in enumerate(loaded_data):
            ...
            # Slope of moving-average data
            # --------------------------------------------------------
            if plot_slope_of_moving_average and plot_moving_average:
    ...
    figure_title = f"{title} Slope"

    if plot_slope_of_raw and plot_slope_of_moving_average:
        figure_title = (
            f"{figure_title} "
            f"[raw slope and MA-{moving_average_window} slope]"
        )
    elif plot_slope_of_raw:
        figure_title = f"{figure_title} [raw slope]"
    elif plot_slope_of_moving_average:
        figure_title = f"{figure_title} [MA-{moving_average_window} slope]"
...
def plot_figure_groups(
    ...
    If calculate_slope=True, a corresponding slope figure is also created
    immediately after each main figure.
    ...
    with PdfPages(pdf_path) as pdf:
        ...
        for group in figure_groups:
            ...
            # Slope figure
            # --------------------------------------------------------
            if group.get("calculate_slope", calculate_slope):

                slope_output_file = make_slope_output_file(
                    group.get("output_file", "plot.png"),
                    output_suffix=output_suffix,
                )

                slope_fig, slope_axes = plot_slope_files(
                    filenames=group["filenames"],
                    selected_vars=group.get("selected_vars", None),
                    delimiter=delimiter,
                    skiprows=group.get("skiprows", skiprows),
                    ...
                    plot_slope_of_raw=group.get(
                        "plot_slope_of_raw",
                        plot_slope_of_raw,
                    ),
                    plot_slope_of_moving_average=group.get(
                ...
                pdf.savefig(slope_fig, bbox_inches="tight")
