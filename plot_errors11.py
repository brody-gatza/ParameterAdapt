...
def get_robust_automatic_y_limits(y):
    ...
    if center is not None and robust_sigma is not None and robust_sigma > 0.0:
        # Loose cutoff so normal noisy data remains visible.
        # Only truly extreme spikes should be rejected.
...
def apply_adaptive_y_limits(
    ...
    for line in ax.get_lines():
        ...
        # Trim endpoints per individual line, not after combining all lines.
        # This handles startup/shutdown spikes without user tuning.
...
def plot_slope_data(
    ...
    for ax, var_index in zip(axes, variable_indices):
        ...
        # Custom robust y-limiting is temporarily disabled for slope plots.
...
def plot_cumulative_slope_data(
    ...
    for ax, var_index in zip(axes, variable_indices):
        ...
        # Custom robust y-limiting is temporarily disabled for cumulative slope plots.
        if APPLY_AUTO_Y_LIMITS_TO_SLOPE_PLOTS:
            apply_limits_if_enabled(
                ax,
                logy=cumulative_slope_logy,
